...
func (r *Repository) ListIncomeReceipts(ctx context.Context, householdID uuid.UUID, fromDate, toDate string) ([]model.IncomeReceipt, error) {
	query := `SELECT id, household_id, income_id, amount_cents, description, income_date::text, auto_generated, voided, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&receipt.ID, &receipt.HouseholdID, &receipt.IncomeID, &receipt.AmountCents, &receipt.Description, &receipt.IncomeDate, &receipt.AutoGenerated, &receipt.Voided, &receipt.CreatedAt, &receipt.UpdatedAt); err != nil {
...
func (r *Repository) CreateIncomeReceipt(ctx context.Context, householdID uuid.UUID, amountCents int64, description, incomeDate string) (*model.IncomeReceipt, error) {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO income_receipts (household_id, amount_cents, description, income_date, auto_generated)
		 VALUES ($1, $2, $3, $4::date, FALSE)
		 RETURNING id, household_id, income_id, amount_cents, description, income_date::text, auto_generated, voided, created_at, updated_at`,
		householdID, amountCents, description, incomeDate,
	).Scan(&receipt.ID, &receipt.HouseholdID, &receipt.IncomeID, &receipt.AmountCents, &receipt.Description, &receipt.IncomeDate, &receipt.AutoGenerated, &receipt.Voided, &receipt.CreatedAt, &receipt.UpdatedAt)
	if err != nil {
...
func (r *Repository) totalExtraIncomeReceiptCents(ctx context.Context, householdID uuid.UUID) (int64, error) {
	var total int64
	err := r.pool.QueryRow(ctx,
		`SELECT COALESCE(SUM(amount_cents), 0) FROM income_receipts
		 WHERE household_id = $1 AND NOT voided AND NOT auto_generated`,
		householdID,
	).Scan(&total)
	if err != nil {
		return 0, fmt.Errorf("sum extra income receipts: %w", err)
	}
	return total, nil
}
